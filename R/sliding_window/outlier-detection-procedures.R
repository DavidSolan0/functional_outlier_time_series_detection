...
multivariate_sliding_window_outlier <- function(
    fdataobj,
    window_size = 8,
    dfunc = "RP",
    threshold = 0.5,
    plot = FALSE) {
  # This function implements outlier detection using sliding window with functional depth
  # and boxplot detection within each window for multivariate functional data
  # params:
  #   fdataobj: List containing multivariate functional data objects
  #   window_size: Size of the symmetric window (default 8)
  #   dfunc: Depth function to use
  #   threshold: Threshold for determining outliers based on window count (default 0.5)
  # returns:
  ...
  # Check if fdataobj is a list
  if (!is.list(fdataobj)) {
    stop("fdataobj must be a list of functional data objects")
  }

  # Get dimensions
  p <- length(fdataobj)  # number of variables
  n <- nrow(fdataobj[[1]])  # number of observations

  # Check that all variables have the same number of observations
  for (i in 1:p) {
    if (nrow(fdataobj[[i]]) != n) {
      stop("All variables must have the same number of observations")
    }
  }

  # Add row names if not present
  if (is.null(row.names(fdataobj[[1]]))) {
    for (i in 1:p) {
      row.names(fdataobj[[i]]) <- 1:n
    }
  }
  ...
  for (i in 1:n) {
    ...
    # Extract window data for each variable
    window_data_list <- list()
    for (v in 1:p) {
      window_data_list[[v]] <- fdataobj[[v]][start_idx:end_idx,]
    }

    # For multivariate case, we need to handle multiple variables
    # We'll detect outliers for each variable separately using functional boxplot
    outlier_indices_in_window <- c()
    
    # Check outliers for each variable separately
    depth_values <- dfunc(window_data_list)
    for (v in 1:p) {
      # Use boxplot to detect outliers in depth values
      boxplot_stats <- fda::fbplot(
        t(window_data_list[[v]]),
        plot = FALSE, depth = depth_values
      )
      var_outliers <- boxplot_stats$outpoint
      
      # Add to overall outliers for this window
      outlier_indices_in_window <- c(outlier_indices_in_window, var_outliers)
    }

    # Get unique outliers in this window
    outlier_indices_in_window <- unique(outlier_indices_in_window)
  ...
  final_outliers <- which(outlier_counts >= min_outlier_windows)
  outliers <- row.names(fdataobj[[1]])[final_outliers]

  if (plot == TRUE) {
    # Plot outlier counts
    par(mfrow = c(1, 1))
    plot(outlier_counts,
      main = "Number of Windows Each Observation is Flagged as Outlier (Multivariate)",
      xlab = "Observation Index",
      ylab = "Number of Outlier Windows",
      col = "steelblue", pch = 19
    )
