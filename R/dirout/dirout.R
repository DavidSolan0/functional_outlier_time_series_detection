DirOut <- function(
    ...
    if (length(temp) == 3) {
        ...
        out_var <- apply(
            ...
            1,
            FUN = function(y) mean(y, na.rm = TRUE)
        ) * d - apply(out_avr^2, 1, FUN = function(y) sum(y, na.rm = TRUE))
        if (D_value) {
            M <- cbind(out_avr, out_var)
            ans <- cov.rob(M, method = "mcd", nsamp = "best", ...)
            cov <- ans$cov
    ...
    if (D_value) {
        if (DirOutmatrix) {
            ...
            return(
                list(D = D, out_avr = out_avr, out_var = out_var, center = me)
            )
        }
    }
    if (!D_value) {
        if (DirOutmatrix) {
            return(
                list(Dirout = Dirout, out_avr = out_avr, out_var = out_var)
            )
        } else {
            return(
                list(out_avr = out_avr, out_var = out_var)
            )
        }
    }
}
